...
- 什么算完成：
  ...
  - 节点点击 = 右侧/浮层抽屉展示该神经元详情与编辑，不抢占主视图。
...
### 3. 节点详情 = 抽屉（非页面）
- 点击画布节点 → 右侧滑出抽屉（宽度 ~320px），内容与 `NeuronDetail` 一致：desc / content / 连接列表 / 时间戳 / tool_ids / 编辑。
...
- `NeuronManager.svelte`：
  ...
  - 布局改为 CSS grid：`[索引侧栏 260px] [画布 1fr]`，抽屉用绝对定位覆盖右侧。
