...
- 我理解当前需求是：在 agent-app GUI 中新增 Topic（话题）管理界面和 Poller（轮询调度器）管理界面。Topic 和 Poller 是 Assistant 模式的核心支撑组件 —— Topic 管理会话的目标和进度，Poller 驱动 Assistant 模式的时间调度。
...
1. **Rust 后端 — 新增 Tauri 命令**
   - `list_topics(status_filter?: string) -> Vec<Topic>`：列出话题，可选按状态过滤
   - `get_topic(id: string) -> Topic`：获取单个话题详情
   - `create_topic(name: string, description?: string) -> Topic`：创建新话题
   - `update_topic(id: string, name?: string, description?: string) -> Topic`：更新话题属性
   - `delete_topic(id: string) -> bool`：删除话题
   ...
   - `pause_topic(id: string) -> Topic`：暂停话题
   - `resume_topic(id: string) -> Topic`：恢复话题
...
2. **Topic 管理界面**
   - 话题列表页面/面板：展示所有话题（ID、名称、状态标签、进度百分比、更新时间）
   - 状态过滤：按全部/todo/in_progress/paused/done/cancelled 筛选
   - 创建新话题：表单（名称、描述），创建后自动跳转到详情
   - 话题详情页：完整展示话题信息（名称、描述、状态、进度、scope items、extra 数据）
...
- **用户操作路径（Topic）**：
  1. 点击侧栏 "Topics" 标签 → 展示话题列表（含状态筛选项）
  2. 点击 "+" 或 "新建话题" 按钮 → 弹出新建表单（名称、描述）→ 提交后创建并进入详情
  3. 点击话题列表项 → 进入话题详情页
  4. 在详情页查看 scope items 列表 → 点击 "添加" 输入 goal + done_contract → 提交后列表更新
  5. 点击 scope item 的 "完成" 按钮 → 该 item 标记完成，进度百分比和话题状态自动更新
  6. 点击 scope item 的 "删除" 按钮 → 二次确认后删除，进度和状态重算
  7. 点击 "暂停话题" 按钮 → 话题状态变更为 paused，scope item 操作禁用
  8. 点击 "恢复话题" 按钮 → 状态根据完成度重新计算
- **用户操作路径（Poller）**：
