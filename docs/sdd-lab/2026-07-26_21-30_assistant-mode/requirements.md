...
- **默认自动轮询（2026-08-01 决策，2026-08-28 翻转）**：Poller 缺省启动即为 `Running` 自动推进；显式 `config.json → poller.enabled: false` 或运行时 `poll_pause` 才暂停（手动步进 `assistant step` / `poll_trigger` 始终可用）。
