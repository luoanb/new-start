...
* 文件域已具备完整 AI 原生文件工具（list/read/write/search\_replace/delete/glob/grep/info/create\_dir/rename），带工作区护栏与已读清单；文档（`workspace-file-management/requirements.md`）明确将「Git 集成」列为暂不处理，本次迭代将其移入范围。
