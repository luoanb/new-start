...
- Assistant mode fixed `system_type` prompt neurons are ensured via `NeuronManager::ensure_system_neuron` / `bootstrap` (startup creates at least `assistant_select_neuron`). All built-in types use the built-in prompt seeds (see `SYSTEM_PROMPT_SEEDS` in `neuron/config.rs`) instead of LLM generation:
  ...
  - `assistant_user_round_judgement`: merged per-user-round judgement — intervention scoring + topic routing (switch / create / none) in one call (lazy ensure)
  - `assistant_round_review`: merged per-round review — scope revision + completion/blocking acceptance in one call (lazy ensure)
- Legacy judgement system types (`assistant_match_topic` / `assistant_score_feedback` / `assistant_complete_scope` / `assistant_revise_topic`) are lazily deprecated: existing rows stay in the database (UI labels fall back to `system_type`), but they are no longer rebuilt by `rebootstrap` and are not invoked by hooks.
