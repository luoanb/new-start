...
`pulsar` is a Rust-core application with multiple entrypoints. The business behavior is implemented once in the Rust core and exposed through Tauri GUI, CLI, and TUI adapters.
...
Tool manuals for **decision-making model readers** live under `packages/pulsar-app/src-tauri/inserts/<id>.md` (scheme 2), loaded by `InsertCatalog` (`rust-embed`).
