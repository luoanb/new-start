...
实现：`packages/agent-app/src-tauri/src/core/model_call_input.rs`。

Insert 文件目录：`packages/agent-app/src-tauri/inserts/<id>.md`（`rust-embed` 编译进二进制）。调用方用 `InsertCatalog::require` 取说明书正文，传入 `assemble` 的 `content`（`Manual`），**不再**用 `system_with_insert` 把 insert 焊进 `role_system`（该 helper 仍保留供其它用途）。
