...
impl ToolRegistry {
    ...
    pub fn with_defaults_and_topics(
        _topic_store: Arc<Mutex<crate::stores::topic_store::TopicStore>>,
    ) -> Self {
        Self::new()
    }
    ...
    pub fn with_defaults_and_topics_and_neurons(
        _topic_store: Arc<Mutex<crate::stores::topic_store::TopicStore>>,
        _neuron_manager: Arc<crate::policies::neuron::manager::NeuronManager>,
        _session_tracker: crate::application::session_tracker::SessionTracker,
    ) -> Self {
        Self::new()
    }
...
/// `ToolCatalog` 端口实现：注册表加读锁转发（方法均同步，锁不跨 await）。
impl crate::core::round_contract::ToolCatalog for std::sync::RwLock<ToolRegistry> {
