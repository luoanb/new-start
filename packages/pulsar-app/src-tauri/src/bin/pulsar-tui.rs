...
async fn main() {
    ...
    let _guard = match pulsar_app_lib::tui::TerminalGuard::new() {
        Ok(g) => g,
        Err(error) => {
            eprintln!("Failed to initialize terminal: {error}");
            let _ = pulsar_app_lib::tui::restore_terminal();
            std::process::exit(1);
        }
    };

    // Create ratatui terminal backend
    let mut terminal = match pulsar_app_lib::tui::init_terminal() {
        Ok(t) => t,
        Err(error) => {
    ...
    let mut app = match pulsar_app_lib::tui::app::TuiApp::new(gateway) {
        Ok(a) => a,
        Err(error) => {
            eprintln!("error [{}]: {}", error.code(), error);
            std::process::exit(error.exit_code());
