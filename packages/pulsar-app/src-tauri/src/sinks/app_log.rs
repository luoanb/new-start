...
pub fn init(
    ...
    let default_level = std::env::var("PULSAR_LOG")
        .ok()
...
pub fn phases() -> &'static [crate::core::log_phase::PhaseInfo] {
    crate::core::log_phase::all_phases()
}
...
mod tests {
    ...
    fn size_rotating_writer_rolls_files() {
        let dir = std::env::temp_dir().join(format!("pulsar-log-test-{}", now_ms()));
