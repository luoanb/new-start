        ...
        for mut conversation in self.list_conversations()? {
            ...
            for message in conversation.messages.iter_mut() {
                ...
                message.map_content(|content| match tool_name.as_deref() {
                    Some(name) => crate::core::context_safety::cap_tool_result(name, content.to_string(), max_chars),
                    None => crate::core::context_safety::cap_text(content, max_chars),
                });
                changed = true;
            }
            if changed {
...
mod tests {
    ...
    fn update_last_assistant_message_patches_text() {
        let store = JsonConversationStore::new(test_root("update_assistant_patches")).unwrap();
        let conv = store
    ...
    fn update_last_assistant_message_no_assistant_is_noop() {
        let store = JsonConversationStore::new(test_root("update_assistant_noop")).unwrap();
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
        store
    ...
    fn update_last_assistant_message_empty_conversation_is_noop() {
        let store = JsonConversationStore::new(test_root("update_assistant_empty")).unwrap();
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
        let updated = store
    ...
    fn concurrent_stream_write_and_user_add_do_not_lose_messages() {
        let store = Arc::new(
            JsonConversationStore::new(test_root("concurrent_stream_write")).unwrap(),
        );
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
    ...
    fn streaming_write_by_index_does_not_overwrite_new_placeholder() {
        let store = JsonConversationStore::new(test_root("stream_write_indexed")).unwrap();
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
        let id = conv.id.clone();
    ...
    fn sanitize_trims_oversized_and_leaves_short_untouched() {
        let store = JsonConversationStore::new(test_root("sanitize_oversized")).unwrap();
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
        let id = conv.id.clone();
    ...
    fn history_page_slices_from_latest() {
        let store = JsonConversationStore::new(test_root("history_page")).unwrap();
        let conv = store
            .create_conversation(None, ConversationMode::Chat)
            .unwrap();
        for (i, role) in [
    ...
    fn list_conversation_summaries_light_parse_and_paginate() {
        let store = JsonConversationStore::new(test_root("summary_light")).unwrap();
    ...
    fn conversation_count_counts_files_only() {
        let store = JsonConversationStore::new(test_root("conversation_count")).unwrap();
...
// ---------------------------------------------------------------------------
// 持久化端口实现（M2，架构设计 §3.5）：为 JSON 存储提供契约端口视图。
// 同步原语（核心管线实际使用）直接转发固有方法；异步三原语（契约面）在同步原语之上组合。
// 追加 / 就地更新 / 写回语义与固有方法完全一致；输入先落库、产物后落库的
// 调用顺序约定由核心管线负责，端口只提供追加原语。
// ---------------------------------------------------------------------------

#[async_trait::async_trait]
impl crate::core::round_contract::ConversationStore for JsonConversationStore {
    fn require_conversation(&self, id: &str) -> AppResult<Conversation> {
        JsonConversationStore::require_conversation(self, id)
    }

    fn add_message(&self, id: &str, message: Message) -> AppResult<Conversation> {
        JsonConversationStore::add_message(self, id, message)
    }
    ...
    fn update_message_at(
        ...
        JsonConversationStore::update_message_at(self, id, index, patch)
    }

    fn save_conversation(&self, conversation: &Conversation) -> AppResult<()> {
        JsonConversationStore::save_conversation(self, conversation)
    }

    async fn load(
        &self,
        id: &crate::core::round_contract::ConversationId,
    ) -> AppResult<crate::core::round_contract::SessionSnapshot> {
        Ok(crate::core::round_contract::SessionSnapshot::from_conversation(
            &self.require_conversation(id.as_str())?,
        ))
    }
    ...
    async fn append_input(
        ...
        for item in items {
            self.add_message(id.as_str(), item.clone())?;
        }
        Ok(())
    }
    ...
    async fn append_outcome(
        &self,
        id: &crate::core::round_contract::ConversationId,
        outcome: &crate::core::round_contract::PersistedOutcome,
    ) -> AppResult<()> {
        for message in &outcome.messages {
            self.add_message(id.as_str(), message.clone())?;
        }
        Ok(())
    }
    ...
    async fn update_text_at(
        ...
        id: &crate::core::round_contract::ConversationId,
        index: usize,
        update: crate::core::round_contract::MessageTextUpdate,
    ) -> AppResult<()> {
        self.update_message_at(id.as_str(), index, |message| {
            ...
            } = &mut message.body
            {
                ...
                *tool_calls = update.tool_calls.clone();
            }
        })?;
        Ok(())
    }
