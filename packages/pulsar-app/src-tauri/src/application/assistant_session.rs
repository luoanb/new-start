...
use crate::core::{
    round_service::{
        read_session_state, set_session_state, ConversationRunner, InputRecord, RoundContext,
        RoundTriggerKind,
    },
    error::{AppError, AppResult},
    models::{
        ChatModelSelection, ChatResponse, ConversationMode, EnsureSystemOpts, Message,
        MessageBody, MessageRole, ResponseFormatSpec, ScopeInItem, ThinkingConfig, Topic,
        TopicStatus, TopicUpdate,
    },
    round_contract::{
        ConversationId, RoundDriver, RoundMode, RoundRequest, RoundService, StreamDelta,
    },
    round_types::{RoundProduct, SessionSeed},
};
use crate::infra::config::config_generation_value;
...
pub use crate::application::hook::instances::{SYSTEM_TYPE_ROUND_REVIEW, SYSTEM_TYPE_USER_ROUND_JUDGEMENT};
...
pub struct AssistantSession {
    pub(crate) store: JsonConversationStore,
    ...
    /// 应用驱动层（架构设计 §5.1）：阻塞路径经驱动发起（依赖 `dyn RoundService`）；
    /// 流式路径暂直走 runner（契约流式变体待端口化）。
    assistant_driver: AssistantDriver,
    poller_driver: PollerDriver,
...
impl AssistantSession {
    ...
    pub async fn converse(
        ...
        let response = match self
            ...
            .run(RoundRequest {
                ...
                input: InputRecord::User(user_input.to_string()),
                mode: RoundMode::Assistant,
            })
            .await
    ...
    pub async fn step(
        ...
        // 模型选型归会话：会话已选模型 → 用会话的（忽略应用默认）；未选 → 写回应用默认。
        self.ensure_session_model(session_id, model)?;
        let response = match self
            .assistant_driver
            .run(RoundRequest {
                session_id: ConversationId::new(session_id),
                input: InputRecord::None,
                mode: RoundMode::Assistant,
            })
            .await
            .map(|outcome| outcome.response)
    ...
    /// run 前确保会话已选定模型（模型选型归会话）：会话已有 → 保留（忽略应用解析出的默认）；
    /// 会话未选定 → 把应用解析出的默认写回会话态，供核心 `load_context` 读取。
    fn ensure_session_model(
        &self,
        session_id: &str,
        ...
        state.model = Some(fallback.clone());
    ...
    pub async fn step_poller(
        ...
        self.poller_driver
            .run(RoundRequest {
                session_id: ConversationId::new(session_id),
                input: InputRecord::Nudge,
                mode: RoundMode::Poller,
            })
            .await
            .map(|outcome| outcome.response)
    }
...
pub(crate) fn is_settling_round(outcome: &RoundProduct) -> bool {
...
mod tests {
    ...
    fn settling_outcome(
        tool_calls: Option<Vec<crate::core::models::AuthorizedToolCall>>,
        tool_results: Vec<crate::core::round_types::ToolResult>,
    ) -> RoundProduct {
        RoundProduct {
            response: String::new(),
            model_output: None,
    ...
    fn settling_round_requires_no_tool_declaration_nor_result() {
        ...
        assert!(!is_settling_round(&settling_outcome(
            None,
            vec![crate::core::round_types::ToolResult {
    ...
    use crate::application::hook::store::HookJudgementFilter;
    use crate::application::poller::new_shared_poll_parallelism;
    use crate::core::{
        models::{ModelRequest, ModelResponse, ModelMessage, NeuronCreate},
        round_executor::{ModelCaller, RoundExecutor},
        round_resolver::RoundResolver,
        session_coordinator::SessionCoordinator,
    };
    use crate::policies::neuron::{config::NeuronConfigReader, model::NeuronModelCaller};
    ...
    fn judgement_harness(providers_config: &str) -> JudgementHarness {
        ...
        let conversation_store = JsonConversationStore::new(root.clone()).unwrap();
        ...
        let runner = ConversationRunner::new(
            Arc::new(conversation_store.clone()),
            resolver,
            executor,
