...
use crate::application::hook::store::HookJudgementStore;
use crate::core::{
    ...
    models::{
        ...
        ConversationMode, ConversationSummaryPage, Message, MessagePage, ModelInfo, ProviderInfo,
        RuntimeStatus, SkillInfo, SystemPromptStatus, TopicStatus, ToolInfo, ToolSource, ToolTag,
    },
...
use crate::policies::{
    ...
    neuron::{
        ...
        model::DefaultNeuronModelCaller,
        store::NeuronStore,
...
use crate::infra::config::{ConfigStore, GitSection};
...
use crate::core::events::{PollerStatus, StateChange, StateEmitter};
...
impl Gateway {
    pub fn default() -> AppResult<Self> {
        Self::new(JsonConversationStore::default()?)
    }

    pub fn new(store: JsonConversationStore) -> AppResult<Self> {
    ...
    fn build(
        ...
        let agent = AgentSession::new(runner.clone());
    ...
    /// 流式增量在 Gateway 内部转发为 `StateChange::MessageDelta`
    ...
    pub fn set_session_model(
        ...
        let mut state = crate::core::round_service::read_session_state(&conversation);
        state.model = Some(selection.clone());
        crate::core::round_service::set_session_state(&mut conversation, &state);
        self.store.save_conversation(&conversation)
    }
...
mod tests {
    use super::*;
    use crate::core::{
        models::{MessageBody, MessageRole, ModelRequest, ModelResponse, ScopeInItem, AuthorizedToolCall},
    ...
    fn tool_call_response() -> ModelResponse {
        ModelResponse {
            ...
            tool_calls: Some(vec![AuthorizedToolCall {
                id: "call_1".into(),
                name: "echo".into(),
    ...
    struct StreamingModelCaller {
        chunks: Vec<Vec<crate::core::models::StreamChunk>>,
    ...
    impl ModelCaller for StreamingModelCaller {
        ...
        async fn call_model_stream(
            ...
            mut on_chunk: Box<dyn FnMut(crate::core::models::StreamChunk) + Send>,
    ...
    ) -> crate::core::models::StreamChunk {
        use crate::core::models::{SseDelta, StreamChoice, StreamChunk};
        StreamChunk {
            id: "chunk".into(),
    ...
    fn gateway_with_emitter(
        ...
        let store = JsonConversationStore::new(test_root(name)).expect("test store should initialize");
        let mut registry = ToolRegistry::new();
    ...
    async fn agent_loop_converges_after_tool_round() {
        let store = JsonConversationStore::new(test_root("agent_loop_converges_after_tool_round"))
    ...
    async fn agent_loop_hits_max_iterations_guard() {
        let store = JsonConversationStore::new(test_root("agent_loop_hits_max_iterations_guard"))
            .expect("test store should initialize");
        let mut registry = ToolRegistry::new();
        registry.register_source(EchoTool, ToolSource::Config);
    ...
    fn clear_conversation_removes_selected_session() {
        ...
        let cleared = gateway
            .clear_conversation(Some(conversation_id.clone()))
            .expect("conversation should clear");

        assert_eq!(cleared, conversation_id);
    ...
    fn poller_status_available() {
        ...
        assert_eq!(
            ...
            crate::application::poller::DEFAULT_POLLER_BASE_INTERVAL_MS
        );
        assert!(status.task_count >= 1);
        // Default / missing poller.enabled → running (2026-08-28 flip).
        assert_eq!(status.state, crate::core::events::PollerRunState::Running);
    }
    ...
    async fn reassemble_tools_loads_disk_config() {
        ...
        let store = JsonConversationStore::new(root.clone()).expect("test store should initialize");
    ...
    fn test_gateway(name: &str) -> Gateway {
        ...
        let store = JsonConversationStore::new(root).expect("test store should initialize");
