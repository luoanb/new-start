<script lang="ts">
  ...
  async function load() {
    ...
    error = null;
    try {
      const list = (await api.call(c.listNeurons, undefined)) as Neuron[];
      ...
      await Promise.all(
        neurons.map(async (n) => {
          try {
            const cs = (await api.call(c.getConnections, { id: n.id })) as Connection[];
            conns.push(...cs);
          } catch {
            // 忽略单节点拉取失败
          }
  ...
  async function refreshDrawerAndGraph() {
    ...
    try {
      const n = (await api.call(c.getNeuron, { id })) as Neuron;
      drawerNeuron = n;
      neurons = neurons.map((x) => (x.id === id ? n : x));
      const cs = (await api.call(c.getConnections, { id })) as Connection[];
      drawerConns = cs;
      // 更新全局连接快照中该节点相关的边
      allConnections = [
  ...
  async function loadAvailableTools() {
    try {
      availableTools = (await api.call(c.listSkills, undefined)) as {
        name: string;
        description: string;
      }[];
    } catch {
  ...
  async function submitCreate() {
    ...
    createError = null;
    try {
      const created = (await api.call(c.createNeuronPlain, {
        desc,
